...
const STATUS_STYLE: Record<ASNStatus, { badge: string; icon: string; dot: string }> = {
  confirmed:        { badge: "bg-amber-50 text-amber-700 border border-amber-200",  icon: "pi pi-clock",        dot: "bg-amber-400" },
  submitted:       { badge: "bg-green-50 text-green-700 border border-green-200",  icon: "pi pi-check-circle", dot: "bg-green-500" },
            ...
            {asn?.items?.map((line, i) => (
              ...
              >
                ...
                <div className="col-span-2">
                  <p className="font-medium text-foreground text-xs">{line.poNo}</p>
                  <p className="text-muted-foreground text-xs">{line.matCode}</p>
                </div>
                <div className="col-span-3 text-foreground text-xs leading-tight">{line.matDesc}</div>
                <div className="col-span-1 text-right text-foreground">{line.uom.toLocaleString()}</div>
                <div className="col-span-1 text-right font-medium text-foreground">{line.numberOfPackages.toLocaleString()}</div>
                <div className="col-span-2">
                  {line.poItem
                    ? <span className="font-mono text-xs bg-muted px-1.5 py-0.5 rounded">{line.poItem}</span>
                    : <span className="text-red-500 text-xs">Missing</span>}
                </div>
                <div className="col-span-2 text-xs text-muted-foreground">{line.expiryDate?.toLocaleDateString() || "—"}</div>
              </div>
            ))}
          </div>
        </div>
